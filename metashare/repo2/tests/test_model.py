...
from metashare.repo2.supermodel import pretty_xml
...
class ModelTest(TestCase):
    ...
    def testRollbackOnImportError(self):
        ...
        """
        _database = {}
        _models = [x for x in dir(sys.modules['metashare.repo2.models'])
          if x.endswith('_model')]
        
        for _model in _models:
            if hasattr(sys.modules['metashare.repo2.models'], _model):
                _inst = getattr(sys.modules['metashare.repo2.models'], _model)
                _database[_model] = _inst.objects.count()
        
        _broken_fixture = '{0}/repo2/fixtures/broken.xml'.format(ROOT_PATH)
    ...
    def testImportExportRoundtrip(self):
        ...
        """
        _roundtrip = '{0}/repo2/fixtures/roundtrip.xml'.format(ROOT_PATH)
