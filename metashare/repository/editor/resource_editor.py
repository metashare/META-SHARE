...
def publish_resources(modeladmin, request, queryset):
    for obj in queryset:
        ...
        if hasattr(obj, 'storage_object') and obj.storage_object is not None:
            saveLRStats(request.user.username, obj.storage_object.identifier, "", PUBLISH_STAT)
...
def unpublish_resources(modeladmin, request, queryset):
    for obj in queryset:
        ...
        if hasattr(obj, 'storage_object') and obj.storage_object is not None:
            saveLRStats(request.user.username, obj.storage_object.identifier, "", INGEST_STAT)
...
class ResourceModelAdmin(SchemaModelAdmin):
    ...
    def save_model(self, request, obj, form, change):
        ...
        #update statistics
        if hasattr(obj, 'storage_object') and obj.storage_object is not None:
            saveLRStats(request.user.username, obj.storage_object.identifier, "", UPDATE_STAT)            
