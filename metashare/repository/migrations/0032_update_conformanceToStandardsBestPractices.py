...
CONFORMANCETOSTANDARDSBESTPRACTICES_MAP = {'BLM': 'BML'}

class Migration(DataMigration):

    def forwards(self, orm):
        # Validate and update conformanceToStandardsBestPractices values.
        # If a value is not valid, yet we have a mapping
        # for it, get its valid equivalent and update its value.

        for annotationinfo in orm.annotationInfoType_model.objects.iterator():
            standards = []
            for standard in annotationinfo.conformanceToStandardsBestPractices_old:
                if CONFORMANCETOSTANDARDSBESTPRACTICES_MAP.get(standard):
                    standard = CONFORMANCETOSTANDARDSBESTPRACTICES_MAP.get(standard)
                standards.append(standard)
            annotationinfo.conformanceToStandardsBestPractices = standards
            annotationinfo.save()

        for langdescr_encoding_info in orm.languageDescriptionEncodingInfoType_model.objects.iterator():
            standards = []
            for standard in langdescr_encoding_info.conformanceToStandardsBestPractices_old:
                if CONFORMANCETOSTANDARDSBESTPRACTICES_MAP.get(standard):
                    standard = CONFORMANCETOSTANDARDSBESTPRACTICES_MAP.get(standard)
                standards.append(standard)            
            langdescr_encoding_info.conformanceToStandardsBestPractices = standards
            langdescr_encoding_info.save()

        for lex_encoding_info in orm.lexicalConceptualResourceEncodingInfoType_model.objects.iterator():
            standards = []
            for standard in lex_encoding_info.conformanceToStandardsBestPractices_old:
                if CONFORMANCETOSTANDARDSBESTPRACTICES_MAP.get(standard):
                    standard = CONFORMANCETOSTANDARDSBESTPRACTICES_MAP.get(standard)
                standards.append(standard)
            lex_encoding_info.conformanceToStandardsBestPractices = standards
            lex_encoding_info.save()

        for inputinfo in orm.inputInfoType_model.objects.iterator():
            standards = []
            for standard in inputinfo.conformanceToStandardsBestPractices_old:
                if CONFORMANCETOSTANDARDSBESTPRACTICES_MAP.get(standard):
                    standard = CONFORMANCETOSTANDARDSBESTPRACTICES_MAP.get(standard)
                standards.append(standard)            
            inputinfo.conformanceToStandardsBestPractices = standards
            inputinfo.save()
        
        for outputinfo in orm.outputInfoType_model.objects.iterator():
            standards = []
            for standard in outputinfo.conformanceToStandardsBestPractices_old:
                if CONFORMANCETOSTANDARDSBESTPRACTICES_MAP.get(standard):
                    standard = CONFORMANCETOSTANDARDSBESTPRACTICES_MAP.get(standard)
                standards.append(standard)
            outputinfo.conformanceToStandardsBestPractices = standards
            outputinfo.save()
    ...
    models = {
        ...
        'storage.storageobject': {
            ...
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'identifier': ('django.db.models.fields.CharField', [], {'default': "'3e67ff30d4c311e5b261f04da2da8af85b151180250c43458675e0a2bd95f694'", 'unique': 'True', 'max_length': '64'}),
