...
USENLPSPECIFIC_MAP = {u'morphosyntacticTagging': u'morphosyntacticAnnotation-posTagging',
u'posTagging': u'morphosyntacticAnnotation-posTagging', u'parsing': u'deepParsing',
u'summarisation': u'summarization', u'texToSpeechSynthesis': u'textToSpeechSynthesis'}

class Migration(DataMigration):

    def forwards(self, orm):
        # Validate and update useNLPSpecific values.
        # If a value is valid keep it as it is,
        # if it's not valid, yet we have a mapping
        # for it, get its valid equivalent and update its value. If it's
        # neither valid nor we have a mapping, replace it with the default value
        # The webServices value isn't mapped to a new one, thus its going to be
        # deleted

        uses_choices = dict(orm.foreseenUseInfoType_model._meta.get_field('useNLPSpecific').choices)
        for forseenuse_info in orm.foreseenUseInfoType_model.objects.iterator():
            uses = []
            for use in forseenuse_info.useNLPSpecific_old:
                if uses_choices.get(use):
                    uses.append(use)
                elif USENLPSPECIFIC_MAP.get(use):
                    uses.append(USENLPSPECIFIC_MAP.get(use))
            forseenuse_info.useNLPSpecific = list(set(uses))
            forseenuse_info.save()

        uses_choices = dict(orm.actualUseInfoType_model._meta.get_field('useNLPSpecific').choices)
        for actualnuse_info in orm.actualUseInfoType_model.objects.iterator():
            uses = []
            for use in actualnuse_info.useNLPSpecific_old:
                if uses_choices.get(use):
                    uses.append(use)
                elif USENLPSPECIFIC_MAP.get(use):
                    uses.append(USENLPSPECIFIC_MAP.get(use))           
            actualnuse_info.useNLPSpecific = list(set(uses))
            actualnuse_info.save()
    ...
    models = {
        ...
        'storage.storageobject': {
            ...
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'identifier': ('django.db.models.fields.CharField', [], {'default': "'74cdce3ed4a911e5a78ff04da2da8af8eb5d7653c12d455e8d132434618fba85'", 'unique': 'True', 'max_length': '64'}),
