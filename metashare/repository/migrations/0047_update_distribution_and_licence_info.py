...
DEFAULT_LICENCE = u'nonStandardLicenceTerms'
AVAILABILITY_MAP = {u"available-restrictedUse": u"available",
    u"available-unrestrictedUse": u"available",
    u"notAvailableThroughMetaShare": u"availableThroughOtherDistributor"}

LICENCES_TO_RESTRICTIONSOFUSE = {
...
LICENCE_MAP = {u"BSD": u"FreeBSD", u"BSD-style": u"FreeBSD", u"MS-C-NoReD": u"MS-NoReD",
               u"MS-C-NoReD-FF": u"MS-NoReD-FF", u"MS-C-NoReD-ND": u"MS-NoReD-ND",
               u"MS-C-NoReD-ND-FF": u"MS-NoReD-ND-FF", u"other": u"nonStandardLicenceTerms"
               }

RESTRICTIONSOFUSE_MAP = {u'academic-nonCommercialUse': u'nonCommercialUse'}

MAPPED_FIELDS = ["distributionAccessMedium", "downloadLocation",
                 "executionLocation", "attributionText", "fee", "userNature"]

COMPARISON_FIELDS = MAPPED_FIELDS + ["licensor", "distributionRightsHolder",
                     "membershipInfo",  "restrictionsOfUse_old"]

def _get_availability(availability, availability_choices):
    if availability_choices.get(availability):
        return availability
    else:
        return AVAILABILITY_MAP.get(availability)
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """
        - licenceInfo: 1 with 1 licence and multiple distributionAccessMedium, downloadLocation, executionLocation, attributionText, licensor, distributionRigthsHolder and userNature
            - create 1 distributionInfo and simply move all of the above to the distributionInfo
            - if a standard licence, correct restrictionsOfUse element
            - if a non-standard licence, keep restrictionsOfUse as they are
        è one licenceInfo, one distributionInfo
        ...
        - licenceInfo: multiple with 1 licence each
            o create one licenceInfo for each licence
            o if a standard licence, map the restrictionsOfUse element, instead of what is already there;
            o if a non-standard licence, keep the restrictionsOfUse as they are
            o check if all the other elements  are the same in all licenceInfo;
                §  if yes, create one distributionInfo and move all of the elements to the distributionInfo; all the licenceInfo are in the same distributionInfo also
                è multiple licenceInfo, one distributionInfo
                §  if not, create for each licenceInfo create a different distributionInfo; move the elements from each old licenceInfo inside this distributionInfo
                è multiple licenceInfo, multiple distributionInfo

        -  licenceInfo: multiple with multiple licences each
            o create one licenceInfo for each licence
            o if a standard licence, map the restrictionsOfUse element, instead of what is already there;
            o if a non-standard licence, keep the restrictionsOfUse as they are
            o check if all the other elements  are the same in all licenceInfo;
                §  if yes, create one distributionInfo and move all of the elements to the distributionInfo; all the licenceInfo are in the same distributionInfo also
                è multiple licenceInfo, one distributionInfo
                §  if not, create for each group of licenceInfo with the same set of other elements a different distributionInfo; move the elements from each old licenceInfo inside this distributionInfo
                è multiple licenceInfo, multiple distributionInfo
        """
        def update_distributionInfo(distribution_info, licence_info):
            # set the values of fields that were in licenceInfo model
            # to distributionInfo model
            for f in MAPPED_FIELDS:
                setattr(distribution_info, f, getattr(licence_info, f))
            distribution_info.save()
            distribution_info.licensor.add(*licence_info.licensor.all())
            distribution_info.distributionRightsHolder.add(*licence_info.distributionRightsHolder.all())
            distribution_info.membershipInfo.add(*licence_info.membershipInfo.all())
        
        def update_licenceInfo(distribution_info, licence_info):
            licences = licence_info.licence_old
            if len(licences)==1:# first case                
                licence = licences[0]
                # Assign licence
                if u'MSCommons' in licence:
                    licence_info.licence = DEFAULT_LICENCE
                elif LICENCE_MAP.get(licence):
                    licence_info.licence = LICENCE_MAP.get(licence)
                else:
                ...
                if LICENCES_TO_RESTRICTIONSOFUSE.get(licence):
                    if licence_info.restrictionsOfUse != LICENCES_TO_RESTRICTIONSOFUSE.get(licence):
                        ...
                        licence_info.restrictionsOfUse = licence_info.restrictionsOfUse_old
                elif any(restriction for restriction in RESTRICTIONSOFUSE_MAP.keys() \
                         if restriction in licence_info.restrictionsOfUse_old):
                    restrictions = []
                    for restriction in licence_info.restrictionsOfUse_old:
                        if RESTRICTIONSOFUSE_MAP.get(restriction):
                            restriction = RESTRICTIONSOFUSE_MAP.get(restriction)
                        restrictions.append(restriction)
                    licence_info.restrictionsOfUse = restrictions
                else:
                    licence_info.restrictionsOfUse = licence_info.restrictionsOfUse_old
                licence_info.save()
                distribution_info.licenceInfo.add(licence_info)
            else: # second case, i.e many licences
                for count, licence in enumerate(licences):
                    if count > 0:
                        # create clones of licenceInfo instance and change
                        # only the licence and auto increment pk
                        licence_info.pk = None
                        licence_info.save()
                    # Assign licence
                    if u'MSCommons' in licence:
                        licence_info.licence = DEFAULT_LICENCE
        ...
        for distribution_info in orm.distributionInfoType_model.objects.iterator():
            distribution_info.availability = _get_availability(distribution_info.availability, availability_choices)
            licence_infos = distribution_info.licenceinfotype_model_set.all()
            if licence_infos.count() == 0:
                distribution_info.save()
            elif licence_infos.count() == 1:# first two cases
                licence_info = licence_infos[0]                
                # set the values of fields that were in licenceInfo model
                # to distributionInfo model
                update_distributionInfo(distribution_info, licence_info)
                update_licenceInfo(distribution_info, licence_info)
            else: # last two cases
                # Group licenceInfo instances according to COMPARISON_FIELDS fields
                licence_infos_grps_dict = defaultdict(list)
                for licence_info in licence_infos:
                    att_dict = model_to_dict(licence_info,
                                             fields=COMPARISON_FIELDS,
                                             exclude=[])
                    atts = json.dumps(att_dict, sort_keys=True)
                    
                    hash_atts = md5(atts).hexdigest()
                    licence_infos_grps_dict[hash_atts].append(licence_info.pk)

                for count1, licence_infos_grps in enumerate(licence_infos_grps_dict.itervalues()):
                    # if it's not the first licenceInfo group, create another distributionIno instance
                    if count1 != 0:
                        ipr_holders = distribution_info.iprHolder.all()
                        distribution_info.pk = None
                        distribution_info.save()
                        distribution_info.iprHolder.add(*ipr_holders)
                    for count2, licence_info_id in enumerate(licence_infos_grps):
                        licence_info = orm.licenceInfoType_model.objects.get(pk=licence_info_id)
                        # if it's the first licenceInfo instance in the group,
                        # assign the values of the removed fields to
                        # distributionInfo instance
                        if count2 == 0:
                            update_distributionInfo(distribution_info, licence_info)
                        update_licenceInfo(distribution_info, licence_info)
    ...
    models = {
        ...
        'storage.storageobject': {
            ...
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'identifier': ('django.db.models.fields.CharField', [], {'default': "'09058e64d96611e59697f04da2da8af8b8a8918a7f5f41be8d55862c1e00554c'", 'unique': 'True', 'max_length': '64'}),
