...
ANNOTATIONTYPE_MAP = {u'syntacticAnnotation-shallowParsing': u'syntacticAnnotation-constituencyTrees',
                      u'syntacticAnnotation-treebanks': u'syntacticAnnotation-dependencyTrees'}

class Migration(DataMigration):

    def forwards(self, orm):
        # Validate and update annotationType values.
        # If a value is not valid, yet we have a mapping
        # for it, get its valid equivalent and update its value.

        for annotation_info in orm.annotationInfoType_model.objects.iterator():
            annotation_type = annotation_info.annotationType
            if ANNOTATIONTYPE_MAP.get(annotation_type):
                annotation_info.annotationType = ANNOTATIONTYPE_MAP.get(annotation_type)
                annotation_info.save()
 
        for inputinfo in orm.inputInfoType_model.objects.iterator():
            annotation_types = []
            for annotation_type in inputinfo.annotationType_old:
                if ANNOTATIONTYPE_MAP.get(annotation_type):
                    annotation_type = ANNOTATIONTYPE_MAP.get(annotation_type)
                annotation_types.append(annotation_type)    
            inputinfo.annotationType = annotation_types
            inputinfo.save()

        for outputinfo in orm.outputInfoType_model.objects.iterator():
            annotation_types = []
            for annotation_type in outputinfo.annotationType_old:
                if ANNOTATIONTYPE_MAP.get(annotation_type):
                    annotation_type = ANNOTATIONTYPE_MAP.get(annotation_type)
                annotation_types.append(annotation_type)            
            outputinfo.annotationType = annotation_types
            outputinfo.save()
    ...
    models = {
        ...
        'storage.storageobject': {
            ...
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'identifier': ('django.db.models.fields.CharField', [], {'default': "'04f8642ad4bd11e59f07f04da2da8af8917aa2fd1bab490abcd8158a9f7d1354'", 'unique': 'True', 'max_length': '64'}),
