...
MIMETYPE_MAP = {u'XML': u'text/xml', u'MS Word': u'application/msword',
    u'wave/audio': u'audio/wav', u'text/txt': u'text/plain',
    u'application:rdf+xml': u'application/rdf+xml', u'sgml': u'text/sgml',
    u'mp3': u'audio/mpeg3', u'WAV': u'audio/wav', u'txt': u'text/plain', u'xml': u'text/xml',
    u'Plain text': u'text/plain', u'audio/x-wav': u'audio/wav',
    u'audio/mp3': u'audio/mpeg3', u'MS Excel': u'application/vnd.ms-excel',
    u'text': u'text/plain', u'rdf+xml': u'application/rdf+xml',
    u'wav': u'audio/wav', u'text/tsv': u'text/tab-separated-values',
    u'plain/text': u'text/plain', u'US-ASCII': u'other', u'txt/xml': u'text/xml',
    u'HTML': u'text/html', u'txt/plain': u'text/plain', u'Plaintext': u'text/plain',
    u'MSExcel': u'application/vnd.ms-excel', u'MSWord': u'application/msword'}

DEFAULT_VALUE = u'other'

class Migration(DataMigration):

    def forwards(self, orm):
        # Validate and update mimeType values. If a value is valid keep it as
        # it is, if it's not valid, yet we have a mapping
        ...
        # append to to_be_curated file, those resources that couldn't be matched to
        # any choice, that got the default value and need to be curated
        path = "{0}/../misc/tools/migration/to_3_1/resources_to_be_modified.txt".format(ROOT_PATH)
        _file = open(path, 'w')
        _file.write(u"Please modify the resources to be valid.\n" \
            u"The following resources had mimeType value that " \
            u"couldn't be matched to any choice, thus got the default value: 'other':\n")

        # Get mimeType valid choices from inputInfoType_model.mimeType field, as
        # they are the same in all mimeType elements
        mime_type_choices = dict(orm.inputInfoType_model._meta.get_field('mimeType').choices)

        # Validate and update in textNumericalFormatInfoType_model.mimeType
        for textnum_format in orm.textNumericalFormatInfoType_model.objects.iterator():
            mime_type = textnum_format.mimeType.replace(" ", "")
            if mime_type_choices.get(mime_type):
                textnum_format.mimeType = mime_type
            elif MIMETYPE_MAP.get(mime_type):
                textnum_format.mimeType = MIMETYPE_MAP.get(mime_type)
            else:
                write_to_resources_to_be_modified_file(_file, textnum_format,
                    mime_type, u'textNumericalFormatInfo --> mimeType')
                textnum_format.mimeType = DEFAULT_VALUE
            textnum_format.save()

        # Validate and update in textFormatInfoType_model.mimeType
        for text_format in orm.textFormatInfoType_model.objects.iterator():
            mime_type = text_format.mimeType.replace(" ", "")
            if mime_type_choices.get(mime_type):
                text_format.mimeType = mime_type
            elif MIMETYPE_MAP.get(mime_type):
                text_format.mimeType = MIMETYPE_MAP.get(mime_type)
            else:
                write_to_resources_to_be_modified_file(_file, text_format,
                    mime_type, u'textFormatInfo --> mimeType')
                text_format.mimeType = DEFAULT_VALUE
            text_format.save()
        
        # Validate and update in audioFormatInfoType_model.mimeType
        for audio_format in orm.audioFormatInfoType_model.objects.iterator():
            mime_type = audio_format.mimeType.replace(" ", "")
            if mime_type_choices.get(mime_type):
                audio_format.mimeType = mime_type
            elif MIMETYPE_MAP.get(mime_type):
                audio_format.mimeType = MIMETYPE_MAP.get(mime_type)
            else:
                write_to_resources_to_be_modified_file(_file, audio_format,
                    mime_type, u'audioFormatInfo --> mimeType')
                audio_format.mimeType = DEFAULT_VALUE
            audio_format.save()
        
        # Validate and update in videoFormatInfoType_model.mimeType
        for video_format in orm.videoFormatInfoType_model.objects.iterator():
            mime_type = video_format.mimeType.replace(" ", "")
            if mime_type_choices.get(mime_type):
                video_format.mimeType = mime_type
            elif MIMETYPE_MAP.get(mime_type):
                video_format.mimeType = MIMETYPE_MAP.get(mime_type)
            else:
                write_to_resources_to_be_modified_file(_file, video_format,
                    mime_type, u'videoFormatInfo --> mimeType')
                video_format.mimeType = DEFAULT_VALUE
            video_format.save()
                
        # Validate and update in imageFormatInfoType_model.mimeType
        for image_format in orm.imageFormatInfoType_model.objects.iterator():
            mime_type = image_format.mimeType.replace(" ", "")
            if mime_type_choices.get(mime_type):
                image_format.mimeType = mime_type
            elif MIMETYPE_MAP.get(mime_type):
                image_format.mimeType = MIMETYPE_MAP.get(mime_type)
            else:
                write_to_resources_to_be_modified_file(_file, image_format,
                    mime_type, u'imageFormatInfo --> mimeType')
                image_format.mimeType = DEFAULT_VALUE
            image_format.save()
        
        # Validate and update in inputInfoType_model.mimeType
        for inputinfo in orm.inputInfoType_model.objects.iterator():
            mime_types = []
            for mime_type in inputinfo.mimeType_old:
                # remove all spaces
                mime_type = mime_type.replace(" ", "")
                if mime_type_choices.get(mime_type):
                    mime_types.append(mime_type)
                elif MIMETYPE_MAP.get(mime_type):
                    mime_types.append(MIMETYPE_MAP.get(mime_type))
                else:
                    write_to_resources_to_be_modified_file(_file, inputinfo,
                        mime_type, u'inputInfo --> mimeType')
                    mime_types.append(DEFAULT_VALUE)
            inputinfo.mimeType = mime_types
            inputinfo.save()
        
        # Validate and update in outputInfoType_model.mimeType
        for outputinfo in orm.outputInfoType_model.objects.iterator():
            mime_types = []
            for mime_type in outputinfo.mimeType_old:
                # remove all spaces
                mime_type = mime_type.replace(" ", "")
                if mime_type_choices.get(mime_type):
                    mime_types.append(mime_type)
                    ...
                    write_to_resources_to_be_modified_file(_file, outputinfo,
                        mime_type, u'outputInfo --> mimeType')
                    mime_types.append(DEFAULT_VALUE)
            outputinfo.mimeType = mime_types
            outputinfo.save()
    ...
    models = {
        ...
        'repository.inputinfotype_model': {
            ...
            'mimeType_old': ('metashare.repository.fields.MultiTextField', [], {'max_length': '50', 'blank': 'True'}),
        ...
        'storage.storageobject': {
            ...
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'identifier': ('django.db.models.fields.CharField', [], {'default': "'615f438cd57111e5b2cef04da2da8af8f99acd3df0a74d679adf00cc7d036ea6'", 'unique': 'True', 'max_length': '64'}),
            ...
            'modified': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2016, 2, 17, 0, 0)'}),
