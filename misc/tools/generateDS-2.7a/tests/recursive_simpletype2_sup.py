...
class PersonType(GeneratedsSuper):
    ...
    def exportChildren(self, outfile, level, namespace_='', name_='PersonType', fromsubclass_=False):
        if self.personId is not None:
            showIndent(outfile, level)
            outfile.write('<%spersonId>%s</%spersonId>\n' % (namespace_, self.gds_format_integer(self.personId, input_name='personId'), namespace_))
        if self.fname is not None:
            showIndent(outfile, level)
            outfile.write('<%sfname>%s</%sfname>\n' % (namespace_, self.gds_format_string(quote_xml(self.fname).encode(ExternalEncoding), input_name='fname'), namespace_))
        if self.lname is not None:
            showIndent(outfile, level)
            outfile.write('<%slname>%s</%slname>\n' % (namespace_, self.gds_format_string(quote_xml(self.lname).encode(ExternalEncoding), input_name='lname'), namespace_))
